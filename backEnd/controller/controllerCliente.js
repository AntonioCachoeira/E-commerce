...
const atualizarCliente = async (req, res) => {
  const dados = req.body;
  try {
    const pesq = Cliente.findByPk(dados.codCliente);
    if (pesq === null) {
      console.log("Cliente não existente");
      res.status(404).json({ message: "Cliente não existente" });
    } else {
      await Cliente.update(dados, { where: { codCliente: dados.codCliente } });
      const pesq2 = await Cliente.findByPk(dados.codCliente);
